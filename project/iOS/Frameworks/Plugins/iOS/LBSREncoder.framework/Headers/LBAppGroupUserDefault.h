...
/// FairPlayRecord的StartRespond音视频数据
static NSString * const kLBFairPlayRecordStartRespond = @"kLBFairPlayRecordStartRespond";
/// FairPlayRecord的音频MStartPts
static NSString * const kLBFairPlayRecordMStartPts = @"kLBFairPlayRecordMStartPts";
...
@property (nonatomic, copy) NSString *mirrorType;           // 0,乐联局域网镜像，1,游密云镜像 默认局域网镜像，2,fairplay局域网镜像
...
@property (nonatomic, assign) BOOL canSupportRetryConnect;  /**< 是否支持重连 */
/// 重连能力
@property (nonatomic, assign) BOOL reconnectEnable;
/// 重连超时时长
...
/// 是否是主程序配置了镜像，用于过滤掉 quic 帧率调节
@property (nonatomic, assign) BOOL hostAppVideoConfigFPS;
/// 是否是主程序配置了镜像，用于过滤掉 quic 码率调节
@property (nonatomic, assign) BOOL hostAppVideoConfigBitRate;
...
/// 保存用户输入的屏幕码信息，例@{@"screenCode":@"1234",@"action":@"1"};,action==0代表用户输入确定，action==1代表用户取消输入
- (void)addMirrorScreenCodeInfo:(NSDictionary *)screenCodeInfo;
/// 获取用户输入的屏幕码信息，例@{@"screenCode":@"1234",@"action":@"1"};,action==0代表用户输入确定，action==1代表用户取消输入
- (NSDictionary *)getMirrorScreenCodeInfo;
